<template>
    <div class="ncform-radio">
      <RadioGroup
        @on-change="handleHackedValue"
        :value="hackValueType(modelVal)"
        :disabled="disabled"
        v-show="!hidden && !readonly"
        size="small"
      >
        <component :is="'Radio' + (mergeConfig.type === 'button' ? 'Button' : '')"
          v-for="opt in dataSource"
          :key="opt[mergeConfig.itemValueField]"
          :label="hackValueType(opt[mergeConfig.itemValueField])"
          :class="mergeConfig.type === 'radio' && mergeConfig.arrangement === 'v' ? 'is-vertical' : ''"
        >{{opt[mergeConfig.itemLabelField]}}</component>
      </RadioGroup>

      <label v-show="readonly" class="labivu-read">{{labelRead}}</label>
    </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      hackValueType(value) {
        if (this.$parent.schema.type === 'boolean') {
          return Number(value)
        }
        return value
      },

      handleHackedValue(value) {
        if (this.$parent.schema.type === 'boolean') {
          this.modelVal = Boolean(value)
        } else {
          this.modelVal = value
        }
