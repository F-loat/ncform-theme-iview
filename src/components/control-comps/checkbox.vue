<template>
  ...
  >
    <Checkbox
      v-if="mergeConfig.selectAll && !readonly && !isBoolean"
      :class="'check-all'"
      :disabled="disabled"
    ...
    <template v-if="!readonly">
      ...
      >
        <component :is="'Checkbox' + (mergeConfig.type === 'button' ? 'Button' : '')"
    ...
    <label
      ...
      :key="idx"
      :class="['labivu-read', mergeConfig.type === 'checkbox' && mergeConfig.arrangement === 'v' ? 'labivu-vertical' : '']"
    >{{label}}</label>
  </div>
</template>
...
<style lang="scss">
  ...
  .ncform-checkbox {
    ...
    .labivu-read {
      ...
      &.not[labivu-vertical] + .labivu-read {
        margin-left: 20px;
      }
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      isBoolean() {
        return this.$parent.schema.type === 'boolean'
      }
    },
